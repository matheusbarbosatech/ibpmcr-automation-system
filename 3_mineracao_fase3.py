"""
Script Principal da FASE 3: Hub Inteligente de Mineração de Conteúdo (Gemini 1.5 Flash).

Sincroniza os arquivos de transcrição (.txt e .json) do Google Drive,
envia o texto para a API do Google Gemini 1.5 Flash (com Freio ABS de 4.5s / 15 RPM),
cruza o texto com os segmentos para anotar os timestamps exatos (start_sec e end_sec),
...
# Força o terminal a usar UTF-8 (Evita erros com acentos no Windows)
if hasattr(sys.stdout, 'reconfigure'):
    sys.stdout.reconfigure(encoding='utf-8', errors='replace')
if hasattr(sys.stderr, 'reconfigure'):
    sys.stderr.reconfigure(encoding='utf-8', errors='replace')

# Garante que o Python ache a pasta raiz do projeto
BASE_DIR = Path(__file__).resolve().parent
sys.path.append(str(BASE_DIR))

# Carrega variáveis de ambiente do arquivo .env diretamente no os.environ
from dotenv import load_dotenv
load_dotenv(BASE_DIR / ".env", override=True)

# Importações das configurações do ecossistema
...
logger = logging.getLogger("Fase3_Mineracao")
...
def process_pending_batch(state_mgr: MasterPlanManager, miner: ContentMinerLLM, max_items: int = 50, force: bool = False) -> int:
    ...
    pbar = tqdm(items_to_process, desc="Minerando Cultos na IA", unit="culto")
    ...
    for stem, txt_path, json_path, out_json_path in pbar:
        ...
        logger.info(f"\n🧠 [FASE 3] Enviando '{stem}' para o Gemini 1.5 Flash...")
        ...
        if insights_dict:
            ...
            try:
                ...
                logger.info(f"📄 Relatório salvo com sucesso: {out_json_path.name}")
            except Exception as e:
            ...
            logger.info(f"💾 Insights salvos no SQLite para {v_id}.")
...
def main():
    parser = argparse.ArgumentParser(description="Fase 3 - Mineração de Conteúdo via Gemini API com Sincronização do Google Drive")
    ...
    # Puxa a chave diretamente de os.environ
    active_gemini_key = os.environ.get("GEMINI_API_KEY", "")
    miner = ContentMinerLLM(gemini_api_key=active_gemini_key, groq_api_key=GROQ_API_KEY)
