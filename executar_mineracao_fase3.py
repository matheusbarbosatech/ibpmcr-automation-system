"""
PIPELINE DE MINERAÇÃO DA FASE 3 (TEXTRANK + NMS + PLAYLISTS) - IBPM CR AUTOMATION SYSTEM.

Executa a Mineração Extrativa Semântica, Heurística e Agrupamento Temático 100% LOCAL (0% LLM, 0% API).

Passos da Execução:
1. Lê as transcrições da pasta data/audio_podcasts/transcricoes_fase2 (ou data/audio_podcasts/transcricoes).
...
4. Gera o agrupamento de Playlists Temáticas Cross-Sermão (data/playlists_tematicas.json).
5. Realiza os cortes ultrarrápidos via FFmpeg -c copy salvando na pasta data/cortes_finais/.

Uso:
    python executar_mineracao_fase3.py
"""
...
logger = get_logger("ExecutarMineracaoFase3")


def main():
    print("=" * 70)
    print("🚀 IBPM CR AUTOMATION SYSTEM - MINERAÇÃO FASE 3 (TEXTRANK + NMS + PLAYLISTS)")
    print("=" * 70)
    ...
    print("\n" + "🎉" * 20)
    print(f"MINERAÇÃO DA FASE 3 CONCLUÍDA COM SUCESSO!")
